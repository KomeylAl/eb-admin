...
export async function GET(req: NextRequest) {
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_API_URL}api/v1/departments?page=${page}&per_page=${pageSize}&search=${search}`,
      {
        method: "GET",
        headers: {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_API_URL}api/v1/departments`,
      {
        method: "POST",
        headers: {
