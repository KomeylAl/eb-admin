...
/**
 * Old backend had GET /doctors/{id}/panel/tomorrow-sms.
 * New API: fetch tomorrow's appointments, then send SMS via /sms/multi.
 */
export async function GET(
  ...
  try {
    const appsRes = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_API_URL}api/v1/doctors/${id}/appointments/tomorrow`,
      {
        method: "GET",
        headers: {
    ...
    if (phones.length === 0) {
      return NextResponse.json(
        { message: "No clients with phone numbers for tomorrow" },
        { status: 404 }
      );
    }
    ...
    const smsRes = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_API_URL}api/v1/sms/multi`,
      {
        ...
        body: JSON.stringify({
          phones,
          message:
            "یادآوری نوبت کلینیک ابراز: فردا نوبت شماست. لطفاً رأس ساعت تعیین‌شده حضور داشته باشید.",
        }),
      }
    );
